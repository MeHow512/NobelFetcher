...
class FileManager:
    ...
    def save_data_to_excel_file(self, laureates_data: list[dict], file_name: str = "laureates_data.xlsx") -> None:
        """
        Formats data headers and converts lists to strings, which allows them to be saved to a cell in .xlsx file.
        It then saves the data in an .xlsx format file using specific formatting for headers and rows.
        ...
        laureates_data = self._convert_each_list_to_string(laureates_data)

        try:
            df = pd.DataFrame(laureates_data)
            workbook = xlsxwriter.Workbook(file_name)
            worksheet = workbook.add_worksheet(f"Nobel laureates in {self._cfg['api_params']['nobelPrizeYear']} - "
            ...
            for laureate in laureates_data:
                for col_num, cell_data in enumerate(laureate.values()):
                    if row_num % 2 == 0:
                        ...
                        worksheet.write(row_num, col_num, cell_data, odd_cell_format)
                row_num += 1

            # adjusting columns width to content size
            for i, width in enumerate(df.map(lambda x: len(str(x))).max()):
